...
public:
	...
	VertexBuffer<Interface::CharDrawCallType, 4, 0> DrawCallBuffer;

	// 各文字インスタンスの情報
	int InstanceCount;
	std::vector<int> TickToDelete;
	Interface::CharInstanceType* Instances = new Interface::CharInstanceType[maxCharCount];
	VertexBuffer<Interface::CharInstanceType, maxCharCount, 1> InstanceBuffer;
	...
	GraphicalStringDraw(HFONT hFont) {
		...
		DrawCallBuffer = VertexBuffer < Interface::CharDrawCallType, 4, 0>(DrawCalls);
		InstanceBuffer = VertexBuffer<Interface::CharInstanceType, maxCharCount, 1>(Instances);
		TickToDelete = std::vector<int>();
	...
	void Draw() {
		...
		InstanceBuffer.UpdateAndSet(Instances, 0, InstanceCount);
	...
	void Append(std::string content, DirectX::XMVECTOR* pColor, DirectX::XMVECTOR* pPos,float height,int tickToDelete,StrDrawPos drawPos) {
		...
		for (int i = 0; i < content.size(); i++) {
			...
			TickToDelete.push_back(tickToDelete);
